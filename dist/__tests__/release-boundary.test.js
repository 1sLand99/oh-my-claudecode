...
const { assertArchive, assertEvidence, assertMarketplaceConsistency, assertNpmAbsent, assertSlsaProvenance, assertSigstoreFallback, assertTrigger, buildEvidenceFromBytes, classifySigstoreRekorFailure, cliMain, decodeDssePayload, prepareStage, selectSlsaAttestation, verifyRegistry, writeEvidence, } = releaseBoundary;
...
describe('assert-marketplace-consistency', () => {
    ...
    function commitVersionBump(root, version, _sha) {
    ...
    it('fails when docs/CLAUDE.md has duplicate version markers', () => {
        const { root, sha } = createPromotionRepository();
        writeFileSync(join(root, 'docs', 'CLAUDE.md'), `<!-- OMC:VERSION:4.15.0 -->\n<!-- OMC:VERSION:${VERSION} -->\n`);
        execFileSync('git', ['add', '.'], { cwd: root, stdio: 'ignore' });
