...
process.stdout.write(JSON.stringify(result));`;
    ...
    it.each([
        ...
        if (!SECURE_MARKER_SUPPORTED)
            return;
        const checkpoint = writeCheckpoint(project, new Date().toISOString());
...
describe('precompact-restore helper parity (issue #3730 security)', () => {
    ...
    beforeEach(() => {
        ...
        writeFileSync(join(project, '.omc', 'state', 'checkpoints', 'checkpoint-now.json'), JSON.stringify({ created_at: new Date().toISOString(), trigger: 'auto', active_modes: {}, todo_summary: { pending: 1, in_progress: 0, completed: 0 }, wisdom_exported: false }), 'utf-8');
    });
    afterEach(() => {
        rmSync(tempDir, { recursive: true, force: true });
    });
